...
namespace {
    class CXStemAddressVisitor : public boost::static_visitor<bool> {
    private:
        CXStemAddress *addr;
    public:
        CXStemAddressVisitor(CXStemAddress *addrIn) : addr(addrIn) { }
...
bool CXStemAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CXStemAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CXStemAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CXStemAddressVisitor(this), dest);
}

bool CXStemAddress::IsValid() const {
...
CTxDestination CXStemAddress::Get() const {
    if (!IsValid())
...
bool CXStemAddress::GetKeyID(CKeyID &keyID) const {
...
bool CXStemAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CXStemSecret::SetKey(const CKey& vchSecret) {
...
CKey CXStemSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CXStemSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CXStemSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CXStemSecret::SetString(const std::string& strSecret) {
