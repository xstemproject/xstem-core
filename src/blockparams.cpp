// Copyright (c) 2018 The XStem Team
...
void GNTdebug()
{
    ...
    LogPrintf("XStem retargetted using: Terminal-Velocity difficulty curve \n");
    return;
}
...
void VRX_BaseEngine(const CBlockIndex* pindexLast, bool fProofOfStake)
{
       ...
       while(scanblocks < scanheight)
       {
           ...
           //
           // v1.0
           if(pindexBest->GetBlockTime() < 1526826196) // OFF (NOT TOGGLED)
           {
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(nBestHeight > RWRD_FIX_TOGGLE) // Correct block reward payouts
    {
        ...
        if(pindexBest->GetBlockTime() > 1921547200)  // ON (Sat, 17 Mar 2018 05:00:00 GMT-07:00)
        {
