...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXStemAmount"))
        settings.setValue("nAnonymizeXStemAmount", 1000);
    nAnonymizeXStemAmount = settings.value("nAnonymizeXStemAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeXStemAmount"))
        SoftSetArg("-anonymizeXStemamount", settings.value("nAnonymizeXStemAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeXStemAmount:
            return QVariant(nAnonymizeXStemAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeXStemAmount:
            nAnonymizeXStemAmount = value.toInt();
            settings.setValue("nAnonymizeXStemAmount", nAnonymizeXStemAmount);
            emit AnonymizeXStemAmountChanged(nAnonymizeXStemAmount);
            break;
        default:
            break;
        }
