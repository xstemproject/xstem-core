...
//
// Bootup the masternode, look for a 50,000 XTE input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in XStem.conf */
...
{
    CXStemAddress address(collateralAddress);
